...
class WebsiteIntegrator(http.Controller):
    ...
    def get_integrator_references(self, integrator):
        ...
        references = integrator.implemented_partner_ids.sorted(
            key=lambda r: r.implemented_date or '',
            reverse=True)

        references = list(filter(
            lambda x: x.website_published is True, references))
    ...
    def integrators_detail(self, integrator_id, **post):
        ...
        if integrator_id:
            ...
            if integrator.sudo().exists() and\
                ...
                modules_list, developed_module_count = \
                    self.get_integrator_modules_list(integrator)

                references = self.get_integrator_references(integrator)

                sponsorship_lines = integrator.sponsorship_line_ids.sorted(
                    key=lambda r: r.date_end, reverse=True)[:5]

                display_all_modules = True if \
                    developed_module_count > 5 else False
                ...
                return request.render(
                    "website_oca_integrator.integrators", values)
    ...
    def integrator_contributors(self, integrator_id=None, country_name=None,
        ...
        country_domain = ['|', ('membership_state', '=', 'paid'),
                          ('github_login', '!=', False),
                          ("website_published", "=", True),
        ...
        countries = partner.sudo().read_group(
            country_domain,
            ["id", "country_id"], groupby="country_id", orderby="country_id")
        ...
        if country_id:
            ...
            current_country = country.browse(country_id).read(['id', 'name'])[
                0]
            if not any(x['country_id'][0] == country_id for x in countries if
                       x['country_id']):
                countries.append({
                    'country_id_count': 0,
        ...
        base_url = '/integrators/%s/contributors%s' % (
            integrator, '/country/%s' % country_id if country_id else '')

        contributors_count = partner.sudo().search_count(country_domain)

        pager = request.website.pager(url=base_url, total=contributors_count,
                                      page=page,
                                      step=self._references_per_page,
                                      scope=7, url_args=post)

        contributors = partner.sudo().search(country_domain,
        ...
        values = {
            ...
            'integrator': integrator,
            'countries': countries,
            'current_country': current_country and [current_country['id'],
                                                    current_country[
                                                        'name']] or None,
            'current_country_id': current_country and current_country[
                'id'] or 0,
            'pager': pager,
            'post': post,
        ...
        return request.render("website_oca_integrator.contributor_index",
                              values)
