...
export class ResPartnerFormController extends FormControllerWithHTMLExpander {
    ...
    async openSponsorHistoryDialog() {
        ...
        console.log(record.data, record.data["html_field_history_metadata"], record.data["html_field_history_metadata"]?.[versionedFieldName])
        if (!historyMetadata) {
            this.notifications.add(
                escape(_t(
                    "The sponsor lacks any past content to review."
                ))
            );
            return;
        }
